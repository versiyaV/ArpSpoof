...
{
    sd = Socket::open_socket(interface.get_ip());
    Socket::bind_socket(sd, interface.get_name());
}
...
void ARP::_spoof(const Host &target, const std::string &spoof_src_ip, const int sd, const int attack_interval_ms, const std::string &fake_mac_address) const 
{
    ...
    auto last_attack_time = Clock::now(); // получить временную метку
    last_attack_time = {}; // очистить временную метку
